...
struct ui_panel
{
    ...
    struct text_line
    {
        ...
        text_line(std::string const &Data, float4 const &Color) : data(Data), color(Color)
        {
        }
    };

    ui_panel(const int X, const int Y) : origin_x(X), origin_y(Y)
    {
    }
...
class ui_layer
{
  ...
  public:
    ...
    void set_progress(float const Progress = 0.0f, float const Subtaskprogress = 0.0f);
    ...
    void push_back(std::shared_ptr<ui_panel> Panel)
    {
